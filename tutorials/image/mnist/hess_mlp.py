...
def getHessianMLP(n_input, n_hidden, n_output):
    ...
    with g.as_default():
        ...
        with tf.name_scope("linear") as scope:
            ...
            weights = tf.reshape(tf.slice(parameters, begin=[idx_from], size=[n_hidden * n_output], name='linear_chen'),
                                 [n_hidden, n_output])

            idx_from = idx_from + n_hidden * n_output
            biases = tf.reshape(tf.slice(parameters, begin=[idx_from], size=[n_output], name='linear_jie'), [n_output])
